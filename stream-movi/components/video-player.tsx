...
export default function LiveStream({ playlistUrl }: { playlistUrl: string }) {
  ...
  return (
    <div>
      ...
      <div className='mt-2'>
        <button onClick={startBroadcast} disabled={!isLive} className='bg-blue-500 text-white px-4 py-2 rounded-md'>
          Start Watching
        </button>
        <button onClick={stopBroadcast} className='ml-2 bg-red-500 text-white px-4 py-2 rounded-md'>
          Stop
        </button>
      </div>
    </div>
